    ...
    <div className="min-h-screen bg-background flex flex-col lg:flex-row">
      ...
      <aside className="hidden lg:flex flex-col w-60 bg-card border-r border-border shrink-0 h-screen sticky top-0">
        ...
        <div className="px-4 py-4 border-b border-border">
          <div className="flex items-center gap-3">
            ...
            <div className="flex-1 min-w-0">
              <div className="text-sm font-bold text-foreground truncate">{playerTeam?.shortName}</div>
              ...
              {state.coachName && (
                <div className="text-[10px] text-primary truncate">🎽 {state.coachName}</div>
              )}
            </div>
          </div>
        </div>
        ...
        <div className="px-4 py-3 border-t border-border">
          ...
          <div className={`text-sm text-mono font-bold ${net >= 0 ? "text-success" : "text-destructive"}`}>
      ...
      <div className="lg:hidden bg-card border-b border-border px-3 py-2 flex items-center justify-between shrink-0">
        <div className="flex items-center gap-2">
          ...
          <div>
            <span className="text-xs font-bold text-foreground uppercase tracking-wider">{playerTeam?.shortName}</span>
            {state.coachName && <div className="text-[9px] text-primary">🎽 {state.coachName}</div>}
          </div>
        </div>
        <div className="flex items-center gap-3">
          <span className="text-[10px] text-muted-foreground">S{state.season} W{state.week}</span>
          <span className={`text-xs text-mono font-bold ${net >= 0 ? "text-success" : "text-destructive"}`}>
      ...
      <main className="flex-1 overflow-y-auto">
        <div className="max-w-5xl mx-auto px-4 lg:px-8 py-4 lg:py-6 pb-24 lg:pb-6">
          {renderView()}
        </div>
      </main>
