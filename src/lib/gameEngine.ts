...
function getTeamStrength(team: Team, modifier: SeasonModifier | null, coachType?: CoachType | null): number {
  if (team.players.length === 0) return 30;
  // Use the 5 starting players. If fewer than 5 are marked isStarter,
  // fill up to 5 from the highest-overall remaining players.
  let starters = team.players.filter(p => p.isStarter);
  if (starters.length < 5) {
    const nonStarters = team.players
      .filter(p => !p.isStarter)
      .sort((a, b) => b.overall - a.overall);
    const needed = 5 - starters.length;
    starters = [...starters, ...nonStarters.slice(0, needed)];
  }
  const fiveStarters = starters.slice(0, 5);
  let avg = fiveStarters.reduce((sum, p) => sum + p.overall, 0) / fiveStarters.length;
  if (modifier === "injury_crisis") {
    avg -= randomBetween(0, 8);
  }
  // Coach synergy bonus: +5% if starters match coach type
  if (coachType && fiveStarters.length === 5) {
    const strategy = getTeamStrategy(team);
    const hasSynergy =
      (coachType === "attack"      && strategy === "offensive") ||
      (coachType === "defense"     && strategy === "defensive") ||
      (coachType === "playmaking"  && strategy === "playmaking");
    if (hasSynergy) avg *= 1.05;
  }
...
function getTeamStrategy(team: Team): string {
  ...
  if (starters.length < 5) {
    const nonStarters = team.players
      .filter(p => !p.isStarter)
      .sort((a, b) => b.overall - a.overall);
    starters = [...starters, ...nonStarters.slice(0, 5 - starters.length)];
  }
...
export function simulateMatch(
  ...
  homeCoachType?: CoachType | null, awayCoachType?: CoachType | null,
  ...
  const homeStr = getTeamStrength(home, modifier, homeCoachType) * (1 + homeBonus) + randomBetween(0, 6);
  const awayStr = getTeamStrength(away, modifier, awayCoachType) * (1 + awayBonus);
...
export function initializeGame(teamId: string): GameState {
  ...
  return {
    ...
    youthIntakeUsedGuest: false,
    titasSamsonasRecruited: false,
  ...
  for (const div of DIVISIONS) {
    ...
    for (const match of round) {
      ...
      const result = simulateMatch(home, away, simState.seasonModifier,
        home.id === simState.selectedTeamId ? simState.coachType : home.id === guestTeamId ? simState.guestCoachType : null,
        away.id === simState.selectedTeamId ? simState.coachType : away.id === guestTeamId ? simState.guestCoachType : null,
      );
...
export function simulatePlayoffGame(state: GameState, matchupIndex: number): GameState {
  ...
  const result = simulateMatch(team1, team2, state.seasonModifier,
    team1.id === state.selectedTeamId ? state.coachType : null,
    team2.id === state.selectedTeamId ? state.coachType : null,
  );
...
export function processOffseason(state: GameState): GameState {
  ...
  // Note: youthIntakeUsed is reset when entering offseason phase (see simulateWeek/playPlayoffGame).
  // It does not need to be reset here.
  // Preserve Titas Samsonas recruited flag across seasons
  newState.titasSamsonasRecruited = state.titasSamsonasRecruited ?? false;
  ...
  // Automatic promotion/relegation: B Division #1 → promoted, A Division #8 → relegated
  {
    ...
    if (rklSorted.length > 0 && lklSorted.length > 0) {
      ...
      newState.teams = newState.teams.map(t => {
        if (t.id === promotedId) {
          // Promoted to A-league: assign proper wages to any free (salary=0) players
          const updatedPlayers = t.players.map(p => {
            if (p.salary === 0) {
              const newSalary = estimateFairSalary(p.overall, Math.max(1, t.prestige));
              return { ...p, salary: newSalary };
            }
            return p;
          });
          // Recalculate totalWages for finances after promotion wage fix
          const newTotalWages = updatedPlayers.reduce((sum, p) => sum + p.salary, 0);
          if (t.id === state.selectedTeamId) {
            newState.finances = { ...newState.finances, totalWages: newTotalWages };
          }
          return { ...t, division: "rkl" as DivisionId, players: updatedPlayers };
        }
  ...
  newState.schedule = [];
  newState.playoffBracket = [];
  // Reset both per-player youth intake flags for the new season
  newState.youthIntakeUsed = false;
  newState.youthIntakeUsedGuest = false;
