...
import { GameState, FinanceRecord } from "@/lib/types";
...
export interface UseLobbyReturn {
  ...
  createLobby: (teamId: string) => Promise<string>;
  joinLobby: (code: string, teamId: string) => Promise<boolean>;
...
export function useLobby(): UseLobbyReturn {
  ...
  const joinLobby = useCallback(
    async (code: string, teamId: string): Promise<boolean> => {
      setIsCreatingOrJoining(true);
      setError(null);
      try {
        ...
        await update(ref(db, `lobbies/${code}`), { guestTeamId: teamId });
  ...
  const startGame = useCallback(async () => {
    ...
    // FIX: start directly in "regular" phase — no preseason in multiplayer
    const sharedState: GameState = {
      ...initialState,
      phase: "regular",
      selectedTeamId: myTeamId,
  ...
  async function simulateAllPlayoffsAndPush(code: string, data: LobbyFirebaseData) {
    ...
    await update(lobbyRef, {
      gameState: { ...finalState, selectedTeamId: data.hostTeamId },
  ...
  async function startNewSeasonAndPush(code: string, data: LobbyFirebaseData) {
    ...
    const newState = processOffseason(hostState);
