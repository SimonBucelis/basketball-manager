...
import { generateFreeAgents, generateYouthPlayer, tryGenerateTitasSamsonas, SEASON_BONUSES } from "@/lib/gameData";
...
interface GameContextType {
  ...
  startGame: (teamId: string, coachName?: string, coachType?: string) => void;
  playWeek: () => void;
...
export function GameProvider({ children }: { children: React.ReactNode }) {
  ...
  const startGame = useCallback((teamId: string, coachName?: string, coachType?: string) => {
    const gs = initializeGame(teamId);
    setSeasonBonusChoices(rollSeasonBonusChoices());
    setState({ ...gs, seasonBonus: null, coachName: coachName || "Coach", coachType: (coachType as any) || undefined });
  ...
  const startRegularSeason = useCallback(() => {
    if (!state) return;
    setState({ ...state, phase: "regular", week: 0 });
  }, [state]);
  ...
  const playWeek = useCallback(() => {
    ...
    if (newState.phase === "offseason") {
      ...
      if (team) {
        const agents = generateFreeAgents(team.division, true, team.prestige);
        const titas = tryGenerateTitasSamsonas(newState.titasSamsonasRecruited ?? false);
        setFreeAgents(titas ? [...agents, titas] : agents);
      }
  ...
  const makeTransferOffer = useCallback((player: FreeAgent, contractYears: 1 | 2, salary: number): { success: boolean; reason?: string } => {
    ...
    const isTitas = !!player.isTitasSamsonas;
    const { success, newState, reason } = attemptTransfer(state, player, contractYears, salary);
    const updatedState = isTitas && success ? { ...newState, titasSamsonasRecruited: true } : newState;
    setState(updatedState);
    if (success) setFreeAgents(prev => prev.filter(p => p.id !== player.id));
    return { success, reason };
  }, [state]);

  const intakeYouth = useCallback(() => {
    if (!state || state.youthIntakeUsed || state.phase !== "offseason") return;
    const team = state.teams.find(t => t.id === state.selectedTeamId)!;
    const availableSlots = 12 - team.players.length;
    if (availableSlots <= 0) return;
    const playersToAdd = Math.min(Math.floor(Math.random() * 3) + 1, availableSlots);
...
export function MultiplayerGameProvider({
  ...
  const prevPhaseRef = React.useRef<string>(initialGameState.phase);
  useEffect(() => {
    const incoming = {
      ...initialGameState,
      selectedTeamId: lobbyState.myTeamId,
    ...
    setState(incoming);
    if (initialGameState.phase === "offseason" && prevPhaseRef.current !== "offseason") {
      const myTeam = initialGameState.teams.find(t => t.id === lobbyState.myTeamId);
      if (myTeam) {
        const agents = generateFreeAgents(myTeam.division, true, myTeam.prestige);
        const titas = tryGenerateTitasSamsonas(initialGameState.titasSamsonasRecruited ?? false);
        setFreeAgents(titas ? [...agents, titas] : agents);
      }
    }
    prevPhaseRef.current = initialGameState.phase;
  ...
  // Per-player youth intake — host uses youthIntakeUsed, guest uses youthIntakeUsedGuest
  const intakeYouth = useCallback(() => {
    if (!state || state.phase !== "offseason") return;
    const isHost = lobbyState.myRole === "host";
    const alreadyUsed = isHost ? state.youthIntakeUsed : state.youthIntakeUsedGuest;
    if (alreadyUsed) return;

    const team = state.teams.find(t => t.id === state.selectedTeamId)!;
    const availableSlots = 12 - team.players.length;
    if (availableSlots <= 0) return;
    const playersToAdd = Math.min(3, availableSlots);
    const newYouthPlayers = [];
    let totalWages = 0;
    for (let i = 0; i < playersToAdd; i++) {
      const youth = generateYouthPlayer(team.prestige);
    ...
    const flagUpdate = isHost
      ? { youthIntakeUsed: true }
      : { youthIntakeUsedGuest: true };
    syncToFirebase({
      ...state,
      teams: newTeams,
      finances: { ...state.finances, totalWages: state.finances.totalWages + totalWages },
      ...flagUpdate,
    });
  }, [state, lobbyState.myRole, syncToFirebase]);
